...
def do_multiply(expr1, expr2):
    ...
    * expr1 is a Product, and expr2 is a Product
    ...
    if oneIsSum and twoIsSum:
        # expr1 is a Sum, and expr2 is a Sum
        for i in expr1:
            for j in expr2:
                result.append(Product([i, j]).simplify())
        result = Sum(result).simplify()

    if oneIsSum and not twoIsSum:
        # expr1 is a Sum, and expr2 is a Product
        for i in expr1:
            result.append(Product([i, expr2]).simplify())
        result = Sum(result).simplify()

    if not oneIsSum and twoIsSum:
        # expr1 is a Product, and expr2 is a Sum
        for i in expr2:
            result.append(Product([i, expr1]).simplify())
        result = Sum(result).simplify()

    if not oneIsSum and not twoIsSum:
        # expr1 is a Product, and expr2 is a Product
        for i in expr1:
            result.append(i)
        for i in expr2:
